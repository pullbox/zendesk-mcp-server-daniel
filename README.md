...
- setup credentials in `.env` file, refer to [.env.example](.env.example).
  ...
  - **Optional:** `ANTHROPIC_API_KEY` — when set, scan tools generate AI-powered per-ticket narrative summaries using Claude Haiku. Without it, summaries fall back to flag-based text with 500-char comment snippets. Get a key at [console.anthropic.com](https://console.anthropic.com).
...
- Output:
  - Ranked list of per-ticket narrative paragraphs (not a table). Each entry includes a clickable ticket link, risk score, escalation/production status, all trouble flags as plain sentences, engineering updates, and a narrative summary of the comment history.
  - If `ANTHROPIC_API_KEY` is set, the summary is AI-generated by Claude Haiku from the full comment history (up to 10 lines). Otherwise it falls back to flag-based text with 500-char comment snippets.
...
- Output:
  ...
  - Same narrative format as `scan_tickets_in_trouble` — ranked per-ticket paragraphs with clickable links, risk scores, and AI-generated summaries when `ANTHROPIC_API_KEY` is set.
